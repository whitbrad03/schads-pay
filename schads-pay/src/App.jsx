...
// ─── SHIFT FORM ───────────────────────────────────────────────────────────────
function ShiftForm({date,profile,templates,onSave,onCancel,existing}){
  const defSleep=profile.defaultSleepAmt?String(profile.defaultSleepAmt):"";
  const[f,setF]=useState(existing||{date,startTime:"",endTime:"",km:0,brokenShift:-1,sleepover:false,sleeoverAmt:defSleep,isPublicHoliday:isPH(date,profile.state||""),isSleepover:false,soStart:"",soEnd:"",soActiveAfterEnd:"",soAgreement:false,wakeups:[],tid:""});
  ...
  const canSave=f.startTime&&(f.isSleepover?f.soStart&&f.soEnd:f.endTime);
  return <div style={{paddingBottom:80}}>
    <BBar onBack={onCancel} title={`${existing?"Edit":"Add"} Shift — ${date}`}/>
    <div style={{padding:"16px 20px 0"}}>
      <Card>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div><div style={{fontWeight:700,fontSize:15,color:C.text}}>🛏️ Sleepover Shift</div><div style={{fontSize:12,color:C.sub}}>Includes overnight sleepover period</div></div>
          <Toggle on={!!f.isSleepover} onChange={v=>setF(x=>({...x,isSleepover:v}))}/>
        </div>
        {!f.isSleepover?(
          <>
          ...
          <>
            <div style={{background:C.purpleLight,borderRadius:10,padding:"10px 12px",marginBottom:12,fontSize:12,color:C.purple,lineHeight:1.5}}>
              Active hours before &amp; after sleepover are assessed independently for shift loadings (1 June 2026 rules). Overtime applies after 12 total active hours (8h max each side, requires written agreement).
            </div>
            ...
            <div style={{fontSize:12,fontWeight:700,color:C.sub,marginBottom:8,marginTop:4}}>SLEEPOVER PERIOD</div>
            <div style={{display:"flex",gap:10}}>
            ...
            <div style={{fontSize:12,fontWeight:700,color:C.sub,marginBottom:8,marginTop:4}}>ACTIVE HOURS (AFTER SLEEPOVER) — OPTIONAL</div>
            <Inp label="Active end (leave blank if none)" type="time" value={f.soActiveAfterEnd||""} onChange={e=>setF({...f,soActiveAfterEnd:e.target.value})}/>
            ...
            <Div/>
            <div style={{fontSize:12,fontWeight:700,color:C.sub,marginBottom:8}}>WAKEUP EVENTS</div>
            {(f.wakeups||[]).map((wu,i)=>(
              <div key={i} style={{background:C.bg,borderRadius:10,padding:"10px 12px",marginBottom:8}}>
                ...
                {wu.start&&wu.end&&<div style={{fontSize:11,color:C.sub}}>Paid: {Math.max(1,Math.ceil(calcHrs(wu.start,wu.end)))}h minimum · first 2h total at 150%, after at 200%</div>}
              </div>
            ))}
            <button onClick={addWakeup} style={{width:"100%",padding:"10px",borderRadius:10,border:`1.5px dashed ${C.accent}`,background:C.accentLight,color:C.accent,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:8}}>+ Add Wakeup</button>
            <Div/>
          </>
        )}
        ...
        {f.isSleepover&&(
          <>
            ...
            <p style={{fontSize:11,color:C.muted,margin:"0 0 8px",lineHeight:1.5}}>Blank = {profile.defaultSleepAmt?`your default $${profile.defaultSleepAmt}`:`award minimum $${AL.sleepDef}`}</p>
          </>
        )}
      </Card>
      <Btn onClick={()=>onSave(f)} disabled={!canSave}>Save Shift</Btn>
...
function TmplScreen({templates,setTemplates,profile}){
  ...
  if(ed){
    const h=ed.startTime&&ed.endTime?calcHrs(ed.startTime,ed.endTime):0;
    return <div style={{paddingBottom:80}}>
      ...
      <div style={{padding:"16px 20px 0"}}>
        <Card>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:ed.sleepover?12:0}}>
            <div><div style={{fontWeight:600,fontSize:14,color:C.text}}>Includes sleepover</div></div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div style={{flex:1}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                ...
                {t.sleepover&&<span style={{background:C.purpleLight,color:C.purple,borderRadius:12,fontSize:11,fontWeight:700,padding:"2px 8px"}}>Sleepover</span>}
              </div>
            </div>
